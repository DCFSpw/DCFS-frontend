...
export default {
  ...
  initUpload: (data) =>
    apiConfig.post(`/files/upload`, data, { lowPriority: true }),
  completeUpload: (fileUUID, throwException) =>
    apiConfig.post(
      `/files/upload/${fileUUID}`,
      {},
      { throwException, lowPriority: true }
    ),
  uploadBlock: (blockUuid, data) =>
    apiConfig.post(`/files/block/${blockUuid}`, data, { lowPriority: true }),

  downloadFile: (fileUuid) => apiConfig.post(`/files/download/${fileUuid}`),
  downloadBlock: (blockUuid, fileUuid) =>
    apiConfig.get(`/files/block/${blockUuid}`, {
      lowPriority: true,
      withHeaders: true,
