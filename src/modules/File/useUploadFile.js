...
export default function () {
  ...
  const uploadBlock = async (file, block, key) => {
    ...
    do {
      ...
      try {
        result = await fileApi.uploadBlock(
          block.uuid,
          createFormData(file, block, key)
        );
        isSuccess = true;
      } catch (e) {
        error = e;
  ...
  const removeUploadingFile = (volumeUuid, rootUuid, file) => {
    ...
    if (
      data[volumeUuid] &&
      data[volumeUuid][rootUuid] &&
      data[volumeUuid][rootUuid][file.uuid]
    ) {
      ...
      if (!Object.values(data[volumeUuid]).length) delete data[volumeUuid];
    }
  ...
  const createBlocks = async (initBlocks, fileToUpload) => {
    ...
    const resultBlocks = initBlocks.sort((a, b) =>
      a.order > b.order ? 1 : -1
    );
    ...
    for (const block of resultBlocks) {
      blocks.push({
        ...
        data: fileToUpload.slice(
          totalSize,
          Math.min(totalSize + block.size, fileToUpload.size),
          fileToUpload.type
        ),
      });
  ...
  const uploadBlocks = async (file, blocks, blocksToUpload = null) => {
    const promises = blocks
      .filter((block) => {
        ...
        return !!blocksToUpload.find(
          (blockToUpload) => blockToUpload.UUID === block.uuid
        );
      })
  ...
  const upload = async (fileToUpload) => {
    ...
    try {
      ...
      if (result.status === RETRY_UPLOAD_HTTP_CODE) {
        await uploadBlocks(initResult.file, blocks, result.data);
