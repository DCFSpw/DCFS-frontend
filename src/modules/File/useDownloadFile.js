...
const BLOCK_DOWNLOAD_TRIES = 3;
const RETRY_DOWNLOAD_HTTP_CODE = 449;
...
export default function() {
  ...
  const downloadMultiple = async (file, blocks) => {
    return await Promise.all(blocks.map(block => new Promise(async (resolve) => {
  ...
  const download = async (file) => {
    if (downloadingFile.value) {
      $q.notify({ type: 'negative', message: 'Only single file can be downloaded at the time' })
      return
    }
    ...
    try {
      ...
      if (result.blocks.length === 1) {
        ...
        const blockBuffers = await downloadMultiple(returnedFile, result.blocks)
